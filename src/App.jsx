...
import { Plus, Minus, Trash2, X, Pencil, Loader2, Briefcase, Package, Users, Copy, Check, CheckCircle2, Circle, ClipboardCopy } from "lucide-react";
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 4px", borderBottom: "1px dashed var(--line)", opacity: packed ? 0.55 : 1 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
        {otherLuggage && otherLuggage.length > 0 && (
          <select
            onChange={(e) => {
              if (e.target.value) {
            ...
            defaultValue=""
            style={{ fontSize: 11, padding: "2px 4px", background: "var(--paper)", border: "1px solid var(--line)", borderRadius: 4, cursor: "pointer", outline: "none" }}
            title="Przenieś do innego bagażu"
          >
...
function LuggageSection({ luggage, onUpdate, onDelete, removable, allLuggageForPerson, onMoveItem }) {
  ...
  const otherLuggage = allLuggageForPerson.filter((l) => l.id !== luggage.id);
      ...
      <div style={{ padding: "6px 14px 14px" }}>
        {luggage.items.map((item) => (
          <ItemRow
            ...
            onChange={(updated) => onUpdate({ ...luggage, items: luggage.items.map(i => i.id === updated.id ? updated : i) })}
            onDelete={() => onUpdate({ ...luggage, items: luggage.items.filter(i => i.id !== item.id) })}
            onMove={(targetLuggageId) => onMoveItem(item, luggage.id, targetLuggageId)}
          />
        ))}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", flexShrink: 0 }}>
      ...
      >
        ...
        <ClipboardCopy size={13} style={{ opacity: 0.6 }} title="Skopiuj zawartość bagażu od innej osoby" onClick={(e) => { e.stopPropagation(); onCopyLuggage(); }} />
        {removable && (
...
export default function PackingListApp() {
  ...
  const handleCopyLuggage = (targetPersonId) => {
    if (!data || data.people.length < 2) {
      alert("Musisz mieć co najmniej dwie osoby, aby móc skopiować bagaż!");
      return;
    }
    const otherPeople = data.people.filter(p => p.id !== targetPersonId);
    const sourceName = prompt(`Od której osoby chcesz skopiować bagaż?\n(Wpisz imię: ${otherPeople.map(p => p.name).join(", ")})`);
    if (!sourceName) return;

    const sourcePerson = otherPeople.find(p => p.name.toLowerCase() === sourceName.trim().toLowerCase());
    if (!sourcePerson) {
      alert("Nie znaleziono osoby o takim imieniu.");
      return;
    }
    ...
    update((d) => ({
      ...
      people: d.people.map(p => {
        if (p.id === targetPersonId) {
          // Głęboka kopia bagażu zresetowana do stanu nie-spakowanego
          const copiedLuggage = sourcePerson.luggage.map(l => ({
            ...l,
            id: uid(),
            items: l.items.map(it => ({ ...it, id: uid(), packed: false }))
          }));
          return { ...p, luggage: copiedLuggage };
        }
        return p;
      })
    }));
  ...
  const handleMoveItem = (item, fromLuggageId, toLuggageId) => {
    update((d) => ({
      ...d,
      people: d.people.map(p => {
        ...
        return {
          ...p,
          luggage: p.luggage.map(l => {
            if (l.id === fromLuggageId) {
              return { ...l, items: l.items.filter(i => i.id !== item.id) };
            }
            if (l.id === toLuggageId) {
              return { ...l, items: [...l.items, { ...item, id: uid() }] };
            }
            return l;
          })
        };
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 18, flexWrap: "wrap", marginBottom: 26 }}>
              {data.people.map((p) => (
                <PersonTag
                  ...
                  onCopyLuggage={() => handleCopyLuggage(p.id)}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {activePerson.luggage.map((l) => (
                  <LuggageSection
                    key={l.id}
                    luggage={l}
                    allLuggageForPerson={activePerson.luggage}
